...
// ✅ NEW: Accept battle state as props from App.js
export default function TeamBattle({ 
  ...
  // ✅ REMOVED: Local battle and stats state (now using props)
  ...
  // ✅ NEW: Determine initial mode based on props (instant!)
  useEffect(() => {
    ...
    if (activeBattle) {
      ...
      if (activeBattle.status === 'waiting') {
        ...
        if (socket && socketReady) {
          socket.emit('join-team-battle-room', { battleCode: activeBattle.battleCode });
        }
...
useEffect(() => {
  ...
  console.log('🔌 Socket ready, ensuring room is joined:', activeBattle.battleCode);
  socket.emit('join-team-battle-room', { battleCode: activeBattle.battleCode });
}, [socket, socketReady, activeBattle?.battleCode]); // ✅ Now depends on activeBattle

// ✅ CRITICAL: Add cleanup to prevent memory leaks
useEffect(() => {
  return () => {
...
}, [activeBattle?.id, socket]); // Run when battle changes or socket changes
  ...
  }, [socket, socketReady, isLeaving]);

  // Timer countdown
  useEffect(() => {
    if (!activeBattle || activeBattle.status !== 'active' || !activeBattle.endTime) {
      setMatchTimer(0);
      return;
    }
    ...
    const calculateRemaining = () => {
      const now = Date.now();
      const remaining = Math.max(0, Math.floor((new Date(activeBattle.endTime).getTime() - now) / 1000));
      return remaining;
    };
 ...
 const handleBattleCreated = (data) => {
  ...
  if (socket && socketReady) {
    ...
    socket.emit('join-team-battle-room', { battleCode: data.battle.battleCode });
  }
};
...
const handleBattleUpdated = (data) => {
  ...
  console.log('  - Current isLeaving state:', isLeaving);
  console.log('  - Full player data:', JSON.stringify(data.battle?.players, null, 2));
  console.log('  - Current activeBattle:', activeBattle ? 'exists' : 'null');
  
  // ✅ CRITICAL: Don't update if we have no active battle (we were removed)
  if (!activeBattle) {
    console.log('⚠️ Ignoring update because activeBattle is null (removed/left)');
    return;
  }
  ...
  const handleBattleStarted = (data) => {
    ...
    setActiveBattle(data.battle);
    setBattleStats(data.stats || { teamAScore: 0, teamBScore: 0, problemsSolved: { teamA: 0, teamB: 0 } });
    setIsPreparing(false);
    setMode('match');
    setError('');
  };
...
const handleRemovedFromBattle = (data) => {
  ...
  console.log('Handler triggered', Date.now());
  
  // ✅ CRITICAL: Clean up state FIRST before leaving socket room
  // This prevents the auto-rejoin useEffect from triggering
  ...
  // THEN leave socket room (after state is cleaned)
  ...
  const handleCopyRoomId = () => {
    ...
    const handleBattleDeleted = (data) => {
  ...
  const handleMoveToSlot = (teamName, slotIndex) => {
    ...
    const currentPlayer = activeBattle.players.find(p => p.userId === user.id);
    if (!currentPlayer) return;

    // Check if slot is already occupied
    const targetSlot = activeBattle.players.find(p => p.team === teamName && p.position === slotIndex);
    if (targetSlot) {
    ...
    console.log('  - Current team:', currentPlayer.team, 'Position:', currentPlayer.position);

    // Immediate local update for instant UI feedback (optimistic update)
  ...
  const handleLeaveBattle = async () => {
    ...
    try {
      // Call API to leave battle (this will update DB and notify others via socket)
      await api.call(`/team-battle/${activeBattle.id}/leave`, {
        method: 'DELETE'
      });
